...
(def appdata
  (read-json 
    (:body 
      (http-client/get 
        (str fb-helper/facebook-base-url "/" (:client-id facebook-app-info))))))

(defroutes handler
  (GET "/" {headers :headers} 
       (if (not *facebook-auth*)
         (render (index (get headers "host") appdata))
         (let [user (fb-client/get [:me] {:extract :body})
              ...
              (render (index (get headers "host") appdata user friends photos likes appfriends)))))
  (GET "/auth/facebook" [] 
    (if (not *facebook-auth*)
      (throw
